...
def run(*argv):
	...
	parser.add_argument('--dataid', type=str, default='../data/GYAFC_family')
	...
	parser.add_argument('--clsrestore', type=str, default="cls_GYAFC_posw2_best")
	parser.add_argument('--lmrestore', type=str, default="lm_GYAFC_best")
...
def eval_main(args):
	...
	data_arg.fields = {
					...
					"dev_0": OrderedDict([("sent", "SentenceDefault"), ("ref", "SessionDefault")]),
					"dev_1": OrderedDict([("sent", "SentenceDefault"), ("ref", "SessionDefault")]),
	...
	def calc(filename, field_key, domain, haveref=True):
		...
		res.fwppl = np.exp(np.sum(np.concatenate(losses_arr)) / np.sum(np.concatenate(token_nums_arr)))
	...
	with open(args.output, "w") as g:
		metric_names = ["acc", "self_bleu", "ref_bleu", "fwppl", "self_g2", "self_h2", "g2", "h2", "overall"]
		...
		for set_name, domain  in product(["dev", "test"], [0, 1]):
			...
			if filepath:
				...
				g.write("\t".join(map(str, output_value)) + "\n")
